...
void main() {
  ...
  group('starting a route', () {
    test('the first leg is spoken with the instruction and a step count',
        () async {
    ...
    test('without a step counter the route runs on sign reading alone',
        () async {
    ...
    test('without a camera guidance says so and offers manual confirmation',
        () async {
  ...
  group('confirming a landmark', () {
    test('reading the sign advances the leg however few steps were counted',
        () async {
      final bloc = await started();
      stepStream.add(3);
      await pump();
    ...
    test('the confirmation and the next instruction are said in one breath',
        () async {
    ...
    test('arrival is announced in one breath too', () async {
      ...
      expect(
        spoken.where(
          (s) =>
              s.text.contains('Reading Hall') && s.text.contains('arrived'),
        ),
        hasLength(1),
      );
      await bloc.close();
    ...
    test('a landmark can be confirmed by hand when there is no camera',
        () async {
      final bloc = await started(camera: false);
  ...
  group('stairs', () {
    test('a leg into a stairwell is guided by the sign, not the count',
        () async {
      final session = GuidanceSession(
        plan: PlannedRoute.fromRecorded(
  ...
  group('obstacles', () {
    ...
    test('a routine obstacle is the lowest-priority thing guidance says',
        () async {
      final bloc = await started();
      spoken.clear();
  ...
  group('recovery', () {
    test('walking well past the expected count starts a recovery sweep',
        () async {
      final bloc = await started();
      spoken.clear();
    ...
    test('a sign off the route replans from where the user actually is',
        () async {
      ...
      final session = sessionOf(
        ...
        landmarks: [
          ...
          landmark('desk', label: 'HELP DESK', name: 'Help desk'),
          landmark('hall', label: 'READING HALL', name: 'Reading Hall'),
          landmark('lift', label: 'LIFT', kind: LandmarkKind.lift, name: 'Lift'),
        ],
      );
      final bloc = await started(session: session);
    ...
    test('with nowhere to relocalise, guidance asks the user to ask a person',
        () async {
      final bloc = await started();
  ...
  test('ending the route releases the camera, the counter and the voice',
      () async {
    final bloc = await started();
  ...
  test('a route over a plan nobody measured is never spoken in steps',
      () async {
    final bloc = blocOf();
    ...
    verifyNever(
      () => speech.speak(any(that: contains('steps')),
          interrupt: any(named: 'interrupt'), use: any(named: 'use')),
    );
    await bloc.close();
  });
}
